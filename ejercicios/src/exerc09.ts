...
function obtenerAtletas(numAtletas: number): Atleta[] {
  let atletas: Atleta[] = [];
  for (let i = 0; i < numAtletas; i++) {
    let nombreAtleta = readlineSync.question(`Ingrese el nombre del atleta finalista ${i + 1}: `);
    let marcaStr = readlineSync.question(`Ingrese la marca del salto en metros de el atleta ${nombreAtleta}: `);
    let marca = parseFloat(marcaStr);

    if (isNaN(marca) || marca <= 0) {
      console.log('Marca no valida >:(. Intente nuevamente owo');
      return obtenerAtletas(numAtletas);
    }
...
function main() {
  ...
  if (isNaN(numAtletas) || numAtletas <= 0) {
    console.log('Cantidad invalida. Intente de nuevo con un dato valido');
    return;
  }

  let atletas = obtenerAtletas(numAtletas);
  ...
  if (atletas.length >= 2) {
    ...
    console.log(`La atleta con meddalla de plata es ${medallaPlata.nombre} con una marca de ${medallaPlata.marca} metros.`);
  }
